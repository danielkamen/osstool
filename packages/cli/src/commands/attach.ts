...
export const attachCommand: CommandModule<object, AttachArgs> = {
  ...
  handler: async (argv) => {
    ...
    const commentBody = [
      ...
      `| **Iteration cycles** | ${session.iteration_cycles ?? 0} |`,
      `| **Post-insert edit ratio** | ${session.post_insert_edit_ratio ?? "?"} |`,
      `| **Test runs observed** | ${session.test_runs_observed ?? 0} |`,
      `| **Largest paste** | ${session.largest_paste_lines ?? 0} lines |`,
      `| **Paste bursts** | ${session.paste_burst_count ?? 0} |`,
      `| **Editor** | ${editors} |`,
